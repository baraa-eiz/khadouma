...
class ProviderController extends Controller
{
    ...
    /**
     * Search & Filter Provider listings
     */
    public function search(Request $request): Response
    {
        ...
        // Retrieve specific entities for contextual titles/descriptions
        ...
        $selectedArea = null;
        if (!empty($areaSlug)) {
            $selectedArea = $this->areasRepo->findBySlug($areaSlug);
        }

        // Perform repository search
        $providers = $this->providerRepo->search($filters);

        // Single search result -> redirect directly to provider
        ...
        if ($selectedService && $selectedCity) {
            ...
            $seoDesc = 'ابحث عن أفضل ' . $selectedService['display_name_ar'] . ' في ' . $selectedCity['display_name_ar'] . ($selectedArea ? ' (' . $selectedArea['display_name_ar'] . ')' : '') . '. تواصل مباشر، تقييمات، وصور أعمال مجاناً.';
        } elseif ($selectedService) {
            $seoTitle = $selectedService['display_name_ar'] . ' في سوريا';
            $seoDesc = 'دليل أفضل فنيي ' . $selectedService['display_name_ar'] . ' في سوريا. تواصل مباشر بدون أي عمولات.';
        } elseif ($selectedCity) {
    ...
    public function show(Request $request, string $slug): Response
    {
        ...
        if (!empty($provider['service_slug'])) {
            $breadcrumbs[$provider['service_name']] = base_url('search?service=' . $provider['service_slug']);
        }
