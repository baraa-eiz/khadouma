...
/**
 * show.php
 * Services Module Admin Show/Details View
 */
?>
...
<div class="section-header">
    ...
    <div style="display: flex; gap: 8px;">
        ...
        <?php if (!$item['is_deleted']): ?>
            <a href="<?= url('admin/services/' . $item['id'] . '/edit') ?>" class="btn btn-primary">
                تعديل البيانات
            </a>
            <form action="<?= url('admin/services/' . $item['id'] . '/delete') ?>" method="POST" style="margin: 0;">
                <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
                <button type="submit" class="btn btn-danger" data-confirm="هل أنت متأكد من أرشفة (حذف مؤقت) لخدمة '<?= e($item['display_name_ar']) ?>'؟">
                    أرشفة الخدمة
                </button>
            </form>
        <?php else: ?>
            <form action="<?= url('admin/services/' . $item['id'] . '/restore') ?>" method="POST" style="margin: 0;">
                <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
                <button type="submit" class="btn btn-success" data-confirm="هل أنت متأكد من استعادة خدمة '<?= e($item['display_name_ar']) ?>'؟">
                    استعادة الخدمة
                </button>
            </form>
        <?php endif; ?>
...
<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px; margin-bottom: 32px;">
    ...
    <div class="card" style="grid-column: 1 / 2;">
        <div class="card-header">
            <h3 class="card-title">بيانات الخدمة الأساسية</h3>
        </div>
        <div class="card-body" style="padding: 0;">
            <table class="table" style="box-shadow: none; border: none;">
                <tbody>
                    ...
                    <tr>
                        <th style="border-bottom: 1px solid var(--border-color); background: none;">الاسم المختصر</th>
                        <td style="border-bottom: 1px solid var(--border-color);"><?= e($item['short_name_ar']) ?></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <th style="border-bottom: 1px solid var(--border-color); background: none;">اسم الأيقونة</th>
                        <td style="border-bottom: 1px solid var(--border-color); font-family: monospace; font-size: 13px;">
                            <?php
                                $iconMap = [
                                    'icon-cleaning' => '🧹',
                                    'icon-plumbing' => '🚰',
                                ...
                                $emoji = $iconMap[$item['icon']] ?? '🛠️';
                                echo $emoji . ' (' . e($item['icon'] ?: 'لا يوجد') . ')';
                            ?>
                        </td>
                    </tr>
                    <tr>
    ...
    <div class="card" style="grid-column: 2 / 3;">
        <div class="card-header">
            <h3 class="card-title">إعدادات محركات البحث والوصف</h3>
        </div>
        <div class="card-body" style="display: flex; flex-direction: column; gap: 16px;">
            <div>
                <h4 style="font-size: 13px; font-weight: bold; color: var(--text-secondary); margin-bottom: 4px;">الوصف التفصيلي للخدمة:</h4>
                <div style="background-color: var(--bg-hover); padding: 12px; border-radius: var(--radius-sm); font-size: 14px; line-height: 1.6; min-height: 80px; color: var(--text-primary);">
                    <?= $item['description_ar'] ? nl2br(e($item['description_ar'])) : '<em style="color: var(--text-muted);">لا يوجد وصف متوفر.</em>' ?>
                </div>
            </div>
...
<div class="card">
    <div class="card-header">
        <h3 class="card-title">سجل العمليات والتتبع للخدمة (Audit Trail)</h3>
    </div>
    <div class="card-body" style="padding: 0;">
        <?php if (empty($auditLogs)): ?>
            <div style="padding: 24px; text-align: center; color: var(--text-muted); font-size: 14px;">
                لا توجد عمليات مسجلة لهذه الخدمة بعد.
            </div>
        <?php else: ?>
                    ...
                    <?php foreach ($auditLogs as $log): ?>
                        <tr>
                            ...
                            <td>
                                <?php
                                    $actionMap = [
                                        'create_service' => '<span class="badge badge-success">إنشاء الخدمة</span>',
                                        'update_service' => '<span class="badge badge-info">تعديل البيانات</span>',
                                        'delete_service' => '<span class="badge badge-danger">أرشفة (حذف مؤقت)</span>',
                                        'restore_service' => '<span class="badge badge-success">استعادة الخدمة</span>',
                                    ];
                                    echo $actionMap[$log['action']] ?? '<span class="badge">' . e($log['action']) . '</span>';
                                ?>
                            </td>
                            <td style="line-height: 1.6;">
                                <?php if ($log['action'] === 'create_service'): ?>
                                    <span style="color: var(--success); font-weight: 600;">تم إنشاء السجل لأول مرة بالبيانات المدخلة.</span>
                                <?php elseif ($log['action'] === 'update_service' && $log['old_value_json'] && $log['new_value_json']):
                                    ...
                                    foreach ($newArr as $k => $val) {
                                        if (array_key_exists($k, $oldArr) && $oldArr[$k] !== $val) {
                                            $fieldLabel = [
                                                'key' => 'الرمز', 'slug' => 'الرابط اللطيف',
                                                'display_name_ar' => 'الاسم الكامل', 'short_name_ar' => 'الاسم المختصر',
                                                'description_ar' => 'الوصف', 'icon' => 'الأيقونة',
