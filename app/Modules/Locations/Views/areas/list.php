...
<div class="table-container">
    <?php if (empty($items)): ?>
        <div class="empty-state">
            ...
            <h3 class="empty-title">لا توجد مناطق مطابقة</h3>
            <p class="empty-desc">لم نعثر على أي مناطق تطابق معايير البحث الحالية.</p>
        </div>
    <?php else: ?>
        <table class="table">
            <thead>
...
<?php if ($totalPages > 1): ?>
    <div class="pagination-container">
        <div class="pagination-info">
            عرض عناصر من <strong><?= (($currentPage - 1) * $limit) + 1 ?></strong> إلى <strong><?= min($totalItems, $currentPage * $limit) ?></strong> من أصل <strong><?= $totalItems ?></strong> منطقة.
        </div>
        <div class="pagination-buttons">
            <?php if ($currentPage > 1): ?>
                <a href="?page=<?= $currentPage - 1 ?>&keyword=<?= e($keyword ?? '') ?>&city_id=<?= e($city_id ?? '') ?>&is_active=<?= e($is_active ?? '') ?>&is_deleted=<?= e($is_deleted ?? '') ?>&sort_by=<?= e($sort_by) ?>&sort_dir=<?= e($sort_dir) ?>" class="btn btn-secondary" style="padding: 6px 12px; font-size: 13px;">
                    السابق
                </a>
            <?php else: ?>
            ...
            <?php for ($p = 1; $p <= $totalPages; $p++): ?>
                <a href="?page=<?= $p ?>&keyword=<?= e($keyword ?? '') ?>&city_id=<?= e($city_id ?? '') ?>&is_active=<?= e($is_active ?? '') ?>&is_deleted=<?= e($is_deleted ?? '') ?>&sort_by=<?= e($sort_by) ?>&sort_dir=<?= e($sort_dir) ?>" class="btn <?= $currentPage === $p ? 'btn-primary' : 'btn-secondary' ?>" style="padding: 6px 12px; font-size: 13px;">
                    <?= $p ?>
                </a>
            <?php endfor; ?>
