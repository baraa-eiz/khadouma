...
/**
 * list.php
 * Cities Module Admin List View
 */
?>

<div class="section-header">
    <h1 class="section-title">إدارة المدن</h1>
...
<!-- FILTERS TOOLBAR -->
<form method="GET" action="<?= url('admin/cities') ?>" class="toolbar" style="gap: 12px; align-items: flex-end;">
    <div style="display: flex; flex-wrap: wrap; gap: 12px; flex-grow: 1;">
        ...
        <div class="form-group" style="margin-bottom: 0; min-width: 140px;">
            ...
            <select name="is_deleted" id="is_deleted" class="form-control">
                <option value="0" <?= $is_deleted === '0' || $is_deleted === null ? 'selected' : '' ?>>المدن الحالية</option>
    ...
    <div style="display: flex; gap: 8px;">
        ...
        <?php if (!empty($keyword) || $is_active !== null || $is_deleted === '1'): ?>
            <a href="<?= url('admin/cities') ?>" class="btn btn-secondary" style="padding: 0.625rem 1.25rem; color: var(--danger);">
                إلغاء التصفية
            </a>
        <?php endif; ?>
    </div>
...
<div class="table-container">
    <?php if (empty($items)): ?>
        <div class="empty-state">
            ...
            <h3 class="empty-title">لا توجد مدن مطابقة</h3>
            <p class="empty-desc">لم نعثر على أي مدن تطابق معايير البحث الحالية.</p>
        </div>
    <?php else: ?>
        <table class="table">
            <thead>
            ...
            <tbody>
                <?php foreach ($items as $item): ?>
                    <tr>
                        ...
                        <td>
                            <a href="<?= url('admin/cities/' . $item['id']) ?>" style="font-weight: 700; color: var(--text-primary);">
                        ...
                        <td style="text-align: left;">
                            <div style="display: inline-flex; gap: 6px; justify-content: flex-end;">
                                <a href="<?= url('admin/cities/' . $item['id']) ?>" class="btn btn-secondary" style="padding: 4px 8px; font-size: 12px;">
                                    التفاصيل
                                </a>
                                
                                <?php if (!$item['is_deleted']): ?>
                                    <a href="<?= url('admin/cities/' . $item['id'] . '/edit') ?>" class="btn btn-secondary" style="padding: 4px 8px; font-size: 12px; color: var(--primary);">
                                        تعديل
                                    </a>
                                    
                                    <form action="<?= url('admin/cities/' . $item['id'] . '/delete') ?>" method="POST" style="margin: 0;">
                                        <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
                                        <button type="submit" class="btn btn-secondary" style="padding: 4px 8px; font-size: 12px; color: var(--danger);" data-confirm="هل أنت متأكد من أرشفة مدينة '<?= e($item['display_name_ar']) ?>'؟">
                                            حذف
                                        </button>
                                    </form>
                                <?php else: ?>
                                    <form action="<?= url('admin/cities/' . $item['id'] . '/restore') ?>" method="POST" style="margin: 0;">
                                        <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
...
<?php if ($totalPages > 1): ?>
    <div class="pagination-container">
        <div class="pagination-info">
            عرض عناصر من <strong><?= (($currentPage - 1) * $limit) + 1 ?></strong> إلى <strong><?= min($totalItems, $currentPage * $limit) ?></strong> من أصل <strong><?= $totalItems ?></strong> مدينة.
        </div>
