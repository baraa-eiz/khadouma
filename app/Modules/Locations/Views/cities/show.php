...
/**
 * show.php
 * Cities Module Admin Show/Details View
 */
?>
...
<div class="section-header">
    ...
    <div style="display: flex; gap: 8px;">
        ...
        <?php if (!$item['is_deleted']): ?>
            <a href="<?= url('admin/cities/' . $item['id'] . '/edit') ?>" class="btn btn-primary">
                تعديل البيانات
            </a>
            <form action="<?= url('admin/cities/' . $item['id'] . '/delete') ?>" method="POST" style="margin: 0;">
                <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
                <button type="submit" class="btn btn-danger" data-confirm="هل أنت متأكد من أرشفة مدينة '<?= e($item['display_name_ar']) ?>'؟">
                    أرشفة المدينة
                </button>
            </form>
        <?php else: ?>
            <form action="<?= url('admin/cities/' . $item['id'] . '/restore') ?>" method="POST" style="margin: 0;">
                <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
                <button type="submit" class="btn btn-success" data-confirm="هل أنت متأكد من استعادة مدينة '<?= e($item['display_name_ar']) ?>'؟">
                    استعادة المدينة
                </button>
            </form>
        <?php endif; ?>
...
<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px; margin-bottom: 32px;">
    ...
    <div class="card" style="grid-column: 1 / 2;">
        <div class="card-header">
            <h3 class="card-title">بيانات المدينة الأساسية</h3>
        </div>
...
<div class="card">
    <div class="card-header">
        <h3 class="card-title">سجل العمليات والتتبع للمدينة (Audit Trail)</h3>
    </div>
    <div class="card-body" style="padding: 0;">
        <?php if (empty($auditLogs)): ?>
            <div style="padding: 24px; text-align: center; color: var(--text-muted); font-size: 14px;">
                لا توجد عمليات مسجلة لهذه المدينة بعد.
            </div>
        <?php else: ?>
                    ...
                    <?php foreach ($auditLogs as $log): ?>
                        <tr>
                            ...
                            <td>
                                <?php
                                    $actionMap = [
                                        'create_city' => '<span class="badge badge-success">إنشاء المدينة</span>',
                                        'update_city' => '<span class="badge badge-info">تعديل البيانات</span>',
                                        'delete_city' => '<span class="badge badge-danger">أرشفة (حذف مؤقت)</span>',
                                        'restore_city' => '<span class="badge badge-success">استعادة المدينة</span>',
                                    ];
                                    echo $actionMap[$log['action']] ?? '<span class="badge">' . e($log['action']) . '</span>';
                                ?>
                            </td>
                            <td style="line-height: 1.6;">
                                <?php if ($log['action'] === 'create_city'): ?>
                                    <span style="color: var(--success); font-weight: 600;">تم إنشاء السجل لأول مرة بالبيانات المدخلة.</span>
                                <?php elseif ($log['action'] === 'update_city' && $log['old_value_json'] && $log['new_value_json']):
